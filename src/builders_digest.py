...
错误（过于简略）："Claude 支持流式工具调用。"

正确（深入展开）："Anthropic 宣布 Claude 现已支持在流式输出（Streaming）模式下使用工具调用（Tool Use）功能。此前 Claude 的工具调用仅支持非流式模式，需要等待完整响应返回后才能触发工具。新的流式模式允许智能体（Agent）在生成过程中实时调用外部工具，实现真正的实时交互式工作流。"
...
1. 推文核心主题：用 3-5 句话深入展开推文内容，包含技术细节、数据、背景说明。引用原文用""标注 [1]
...
关键引用：列出播客中最有价值的 2-3 句原话引用，用""标注
...
8. 引用用""标注，加角标 [1][2]
9. 文末列出参考来源链接
10. 禁止使用任何 Markdown 格式（不要用 #、**、*、---、[]()、>、``` 等标记符号）
11. 使用纯文本格式：换行分段、数字编号列表、「」标注术语、""标注引用"""
