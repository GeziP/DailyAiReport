"""推荐系统 - 发现已关注 Builder/播客的相似来源，并自动添加到追踪列表"""
...
class RecommendedSource:
    ...
    url: str
    handle: str  # Twitter handle 或 YouTube channel handle
    reason: str
    topics: List[str]
...
{{
  "recommendations": [
    {{
      ...
      "handle": "Twitter handle (不含@) 或 YouTube channel handle",
      "url": "@handle 或完整 URL",
...
注意：只输出 JSON，不要有其他内容。handle 字段要提取干净的账号名（如 karpathy，不是 @karpathy）。"""
...
{json.dumps(sources["podcasts"], indent=2, ensure_ascii=False)}"""
    ...
    def get_sources_config(self) -> dict:
        """获取当前的 sources 配置"""
        sources_file = Config.BASE_DIR / "config" / "follow-builders-sources.json"

        if not sources_file.exists():
            return {"x_accounts": [], "podcasts": []}
    ...
    def is_already_in_sources(self, handle: str, sources: dict) -> bool:
        """检查是否已在 sources 配置中"""
        handle_lower = handle.lower().replace("@", "")

        # 检查 X accounts
        for account in sources.get("x_accounts", []):
            existing_handle = account.get("handle", "").lower().replace("@", "")
            if existing_handle == handle_lower:
                return True

        # 检查 podcasts
        for podcast in sources.get("podcasts", []):
            existing_handle = podcast.get("channelHandle", "").lower()
            if existing_handle == handle_lower:
                return True
    ...
    def recommend(self) -> List[RecommendedSource]:
        ...
        try:
            ...
            for item in result.get("recommendations", []):
                handle = item.get("handle", "") or item.get("handle_or_url", "")
                handle = handle.replace("@", "").strip()

                # 跳过已在 sources 配置中的
                if self.is_already_in_sources(handle, sources):
                    continue
                ...
                recommendations.append(RecommendedSource(
                    ...
                    url=item.get("url", "") or item.get("handle_or_url", ""),
                    handle=handle,
    ...
    def add_to_sources(
        ...
        for rec in recommendations:
            if rec.type == "builder" and rec.platform == "twitter":
                # 添加到 X accounts
                sources["x_accounts"].append({
                    "name": rec.name,
                    "handle": rec.handle
                })
            elif rec.type == "podcast" and rec.platform == "youtube":
                # 添加到 podcasts
                sources["podcasts"].append({
                    "name": rec.name,
                    "type": "youtube_channel",
                    "url": rec.url,
                    "channelHandle": rec.handle
                })
...
def generate_recommendations() -> Optional[List[RecommendedSource]]:
    ...
    for i, rec in enumerate(recommendations, 1):
        ...
        print(f"   平台: {rec.platform} | 账号: {rec.handle}")
        print(f"   理由: {rec.reason}")
        print()

    # 直接添加到 sources 配置（以后自动追踪）
    sources_file = recommender.add_to_sources(recommendations)
    if sources_file:
        print(f"✓ 已添加到追踪列表: {sources_file}")
        print("  以后运行时会自动追踪这些新来源")
